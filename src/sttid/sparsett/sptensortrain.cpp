             ...
             double const spthres, bool check_flag, bool cross_flag) 
{
    ...
    size_t r = 1;                            // Current (Initial) rank
    ...
    for (int i = dim - 1; i > 0; i--) {
        ...
        std::cout << "Before PRRLU decomposition -MEM(MB):Virtual:" << virtualMem/1024.0
...
//template SparseTTRes TT_ID_sparse<double, ...>(const COOTensor<double, 6>&, double, double, size_t, bool);