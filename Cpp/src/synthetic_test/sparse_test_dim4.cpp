...
{ 
    if (argc != 10) {
        std::cerr << "Error: Expected 9 arguments, got " << (argc - 1) << std::endl;
        std::cerr << "Usage: " << argv[0] << " <data_file_path> <nnz> <order1> <order2> <order3> <order4> <rmax> <epsilon> <spthres>" << std::endl;
        return 1;
    }  
