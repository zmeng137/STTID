                        ...
                        double const spthres, long long const maxdim, bool const isFullReturn)
{
    ...
    while (s < k) {
        ...
        size_t memCOO = (sizeof(double)+sizeof(long long)*2) * (Ms.nnz_count+Mp.nnz_count);
        std::cout << "-MEM(MB) of COO Ms+Mp:" << memCOO / 1024.0 / 1024.0;
        ...
        SparseVector<double> vr(Nc, Nc);          // row vector storing the pivoted row 
        SparseVector<double> vc(Nr, Nr);          // col vector storing the pivoted col
